...
app.listen(PORT, () => {
  console.log(`\n🚀 Libragold API running on http://localhost:${PORT}`);
  console.log(`📊 Health check:  http://localhost:${PORT}/health`);
  console.log(`🔐 Auth endpoint: http://localhost:${PORT}/api/auth/login`);

  if (process.env.NODE_ENV === 'production') {
    console.log(`🖥️  Admin UI:      http://localhost:${PORT}/admin`);
    console.log(`📄 Swagger UI:    disabled in production (set SWAGGER_ENABLED=true to enable)`);
  } else {
    console.log(`🖥️  Admin UI:      http://localhost:5174 (cd admin && npm run dev)`);
