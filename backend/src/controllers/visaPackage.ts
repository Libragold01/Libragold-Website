...
export async function listVisaPackages(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    console.error('List visa packages error:', err);
    res.status(500).json({ error: 'Failed to fetch visa packages' });
  }
}

// ─── GET /api/visa-packages/:slug ─────────────────────────────────────────────
export async function getVisaPackage(req: Request, res: Response): Promise<void> {
  try {
    const pkg = await prisma.visaPackage.findUnique({ where: { slug: req.params.slug } });
    if (!pkg) {
      res.status(404).json({ error: 'Visa package not found' });
      return;
    }
    res.json({ package: pkg });
  } catch (err) {
    console.error('Get visa package error:', err);
...
export async function createVisaPackage(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if ((err as { code?: string }).code === 'P2002') {
      res.status(409).json({ error: 'A visa package with this slug already exists' });
      return;
    }
...
// ─── PUT /api/visa-packages/:id ───────────────────────────────────────────────
export async function updateVisaPackage(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    console.error('Update visa package error:', err);
    res.status(500).json({ error: 'Failed to update visa package' });
  }
}

// ─── DELETE /api/visa-packages/:id ───────────────────────────────────────────
export async function deleteVisaPackage(req: Request, res: Response): Promise<void> {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) {
    res.status(400).json({ error: 'Invalid ID' });
    return;
  ...
  try {
    ...
    console.error('Delete visa package error:', err);
